...
const int COLOR_TABLE_SIZE = 6;
const int COLOR_TABLE[] = {RED, GREEN, BLUE, YELLOW, CYAN, MAGENTA};
...
typedef struct {
    ...
    bool pressedDown;   // Needed for rendering when KEY_DOWN
    unsigned int lastx; // Needed for rendering when KEY_DOWN
} gameConfig;
...
/**
 * @brief Maps 24-bit RGB colors to 16-bit RGB colors.
 *        Color assembled in "rrrrrggg gggbbbbb" format.
 *        The color is then written to the framebuffer.
 * @param x x-coordinate of pixel (0 = left)
 * @param y y-coordinate of pixel (0 = top)
 * @param color the 24-bit RGB color which is written to pixel
 */
void setPixel(int x, int y, int color) {
    int offset = (x + y*vScreenInfo.xres)*vScreenInfo.bits_per_pixel/8;
    fbMap[offset + 0] = ((((color & GREEN) >> 8) >> 2) << 5)
                      | ((((color & BLUE) >> 0) >> 3) << 0);
    fbMap[offset + 1] = ((((color & GREEN) >> 8) >> 2) >> 3)
                      | ((((color & RED) >> 16) >> 3) << 3);
}
...
bool initializeSenseHat() {
    for (int i = 0; i < FB_MAX; i++) {          // FB_MAX is currently 32
        char filepath[10];                      // Supports /dev/fb0 to /dev/fb31
        memset(filepath, 0, sizeof(filepath));  // Clear the buffer
        sprintf(filepath, "/dev/fb%d", i);
        fbfd = open(filepath, O_RDWR);
        if (fbfd == -1)
            return false;   // Couldn't open framebuffer
        if (ioctl(fbfd, FBIOGET_FSCREENINFO, &fScreenInfo) == -1)
            return false;   // Couldn't get fixed screen information
        if (ioctl(fbfd, FBIOGET_VSCREENINFO, &vScreenInfo) == -1)
            return false;   // Couldn't get variable screen information
        if (!strcmp(fScreenInfo.id, "RPi-Sense FB"))
            break;          // found the framebuffer
        close(fbfd);
    }
    fbMap = mmap(0, fScreenInfo.smem_len, PROT_WRITE, MAP_SHARED, fbfd, 0);
    if (fbMap == MAP_FAILED)
        return false; // mmap failed
    for (int i = 0; i < 32; i++) {
        char deviceName[32];
        char filepath[18];                      // Supports /dev/event0 to /dev/event31
        memset(filepath, 0, sizeof(filepath));  // Clear the buffer
        sprintf(filepath, "/dev/input/event%d", i);
        eventfd = open(filepath, O_RDONLY);
        if (eventfd == -1)
            return false;   // Couldn't open event device
        if (ioctl(eventfd, EVIOCGNAME(sizeof(deviceName)), deviceName) == -1)
            return false;   // Couldn't get name of device
        if (!strcmp(deviceName, "Raspberry Pi Sense HAT Joystick"))
            break;          // found the joystick
        close(eventfd);
    }
    return true;
}
...
void freeSenseHat() {
    munmap(fbMap, fScreenInfo.smem_len);
    close(fbfd);
    close(eventfd);
}
...
static inline bool tileOccupied(coord const target); // Need to be declared before use
...
void renderSenseHatMatrix(bool const playfieldChanged) {
    if (!playfieldChanged)
        return;
    static int color;
    if ((game.activeTile.x == 3) && (game.activeTile.y == 0))
        color = COLOR_TABLE[rand() % COLOR_TABLE_SIZE]; // Get random color if new tile
    for (unsigned int y = 0; y < game.grid.y; y++) {
        for (unsigned int x = 0; x < game.grid.x; x++) {
            coord const checkTile = {x, y};
            if (tileOccupied(checkTile)) {
                bool isActiveTile = (game.activeTile.x == x) && (game.activeTile.y == y);
                bool isPressedDownTile = (game.pressedDown) && (game.lastx == x);
                if (isActiveTile) {             // Make sure to only color active tile
                    setPixel(x, y, color);
                } else if (isPressedDownTile) { // Need this case because of KEY_DOWN, which does
                    game.pressedDown = false;   // not count the downed tile as an active tile
                    setPixel(x, y, color);
                }
            } else {
                setPixel(x, y, 0x0);
...
bool sTetris(int const key) {
    ...
    if (game.state & ACTIVE) {
        ...
        if (key) {
            ...
            case KEY_DOWN:
                while (moveDown()) {};
                game.lastx = game.activeTile.x; // Remember the last column of downed tile
                game.pressedDown = true;        // Flag that a tile was downed
                game.tick = 0;
                break;
            default:
